# coding: utf-8

"""
    CircleCI API

    This describes the resources that make up the CircleCI API v2.

    The version of the OpenAPI document: v2
    Generated by: https://konfigthis.com
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING
from pydantic import BaseModel, Field, RootModel

from circle_ci_python_sdk.pydantic.insights_get_project_workflow_metrics_response_items_item_metrics import InsightsGetProjectWorkflowMetricsResponseItemsItemMetrics

class InsightsGetProjectWorkflowMetricsResponseItemsItem(BaseModel):
    # The name of the workflow.
    name: str = Field(alias='name')

    metrics: InsightsGetProjectWorkflowMetricsResponseItemsItemMetrics = Field(alias='metrics')

    # The timestamp of the first build within the requested reporting window.
    window_start: datetime = Field(alias='window_start')

    # The timestamp of the last build within the requested reporting window.
    window_end: datetime = Field(alias='window_end')

    # The unique ID of the project
    project_id: typing.Union[bool, date, datetime, dict, float, int, list, str, None] = Field(alias='project_id')
    class Config:
        arbitrary_types_allowed = True
